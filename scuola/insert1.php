...
function insert_scuola(){
    if(isset($_POST['codice'])&&isset($_POST['nome'])&&isset($_POST['indirizzo'])&&isset($_POST['numero'])&&isset($_POST['tipotel'])){
        ...
        if(!$check_result1)
        {
            echo "Errore: ".pg_last_error($con)."<br><a href='index.php'>Torna alla pagina index per le scuole.";
            exit;
        }
        if(!$check_result2)
        {
        ...
        if($media&&!isset($_POST['anno'])){
            echo "Errore: inserire anno di fondazione<br><a href='index.php'>Torna alla pagina index per le scuole.";
            exit;
        }
        $query=null;
        if($infanzia)
        ...
        if($result==TRUE) echo "<div>Inserimento scuola avvenuto con successo</div><br><a href='index.php'>Torna alla pagina index per le scuole.";
        else echo "<div>'Inserimento scuola fallito</div><br><a href='index.php'>Torna alla pagina index per le scuole.";
