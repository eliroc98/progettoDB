...
function edit_alunno(){
    if(isset($_POST['cf'])){
        ...
        if(!$queryTipo_res)
        {
            echo "Errore: ".pg_last_error($con)."<br><a href='index.php'>Torna alla pagina index per gli alunni.";
            exit;
        }
        ...
        if($gen1=='no')
        {
            ...
            if($gen1==$cfG1){
                ...
                $num= pg_fetch_assoc($queryTel_res)["numero"];
                if($num!=$numeroG1){
                    $query.="INSERT INTO Telefono VALUES('$numeroG1','$tipotelG1');";
                    $query.="INSERT INTO ContattoG VALUES('$cfG1','$numeroG1');";
                }
                ...
                $queryCred_res = pg_query($con,$queryCred);
                if(!$queryCred_res){
                    echo "Errore: ".pg_last_error($con)."<br><a href='index.php'>Torna alla pagina index per gli alunni.";
                    exit;
                }
                $ut = pg_fetch_assoc($queryCred_res)["nomeutente"];
                ...
                if(pg_num_rows($queryGen_res)>0){
                    $gg=pg_fetch_assoc($queryGen_res)["genitore"];
        ...
        if($chkgen2=="si"){
            ...
            if($gen2=='no'){
                ...
                if($gen2==$cfG2){
                    ...
                    $queryGen_res = pg_query($con, $queryGen);
                    if(pg_num_rows($queryGen_res)>0){
                        $gg=pg_fetch_assoc($queryGen_res)["cfG"];
        ...
        if($query_res) echo "<div>Modifica alunno avvenuta con successo</div><br><a href='index.php'>Torna alla pagina index per gli alunni.";
        else echo "<div>Modifica alunno fallita</div><br><a href='index.php'>Torna alla pagina index per gli alunni.";
    }
    else echo "<div>Modifica alunno fallita</div><br><a href='index.php'>Torna alla pagina index per gli alunni.";
}
edit_alunno();
